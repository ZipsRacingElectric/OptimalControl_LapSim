...
# Problem Variables
@infinite_parameter(m, t in [0, finalTimeInitialGuess], num_supports = 121)
@variables(m, begin
    # state variables
    x[1:2], Infinite(t)
    v[1:2], Infinite(t)
...
throttleMap(u,v) = 1.1u + .4v
breakMap(u) = .5u

throttleBreak(u,v) = InfiniteOpt.ifelse(u ≥ 0, throttleMap(u,v), breakMap(u))
...
@constraint(m, [i = 1:2], ∂(v[i], t) == (tf/finalTimeInitialGuess) * throttleBreak(u[i],v[i]))
